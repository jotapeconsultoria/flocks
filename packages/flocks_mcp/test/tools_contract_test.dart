...
// números aqui (131 componentes, 24·71·36 por categoria) são os do catálogo de
// verdade, e é de propósito que envelheçam junto com ele — quando o `flocks`
// ganhar o 132º, este arquivo fica vermelho e alguém decide conscientemente se
...
void main() {
  group('list_components', () {
    test('devolve os 131 componentes do catálogo', () {
      final Map<String, Object?> out = call('list_components');
      expect(out['count'], 131);
      expect((out['components']! as List<Object?>).length, 131);
      expect(out['lang'], 'en');
    });
    ...
    test('cada item traz id, name e summary — e nada além', () {
      ...
      // devolver as 131 fichas inteiras seriam ~490 KB numa resposta só.
    ...
    test('as três categorias particionam o catálogo', () {
      ...
      final int organisms =
          call('list_components', <String, Object?>{
              ...
              as int;
      expect(<int>[atoms, molecules, organisms], <int>[24, 71, 36]);
      expect(atoms + molecules + organisms, 131);
    });
