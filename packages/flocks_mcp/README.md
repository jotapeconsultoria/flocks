...
The server is not listed on the [MCP
Registry](https://registry.modelcontextprotocol.io) yet — like pub.dev,
publishing there is irreversible, and it goes out deliberately, not as a side
effect of CI. What is decided and left ready:

- **Namespace**: `io.github.jotapeconsultoria/flocks-mcp`, authenticated by
  GitHub device flow. The DNS alternative on `flocks.live` was weighed and
  dropped: reverse-DNS would read `live.flocks/…` (the TLD comes first), and
  the method requires keeping an Ed25519 private key as a standing secret,
  against a device flow that keeps none — and the artifact already lives on
  GitHub Releases.
...
When the time comes, publishing is:
