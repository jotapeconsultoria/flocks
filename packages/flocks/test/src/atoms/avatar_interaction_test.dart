...
void main() {
  ...
  AvatarInteraction resolve(
    Set<WidgetState> states, {
    AppAvatarEffect effect = AppAvatarEffect.scale,
    bool isDark = false,
  ...
  group('resolveAvatarInteraction · escala', () {
    ...
    test('effect none → 1.0 mesmo em hover/press', () {
      expect(
        resolve(<WidgetState>{
          WidgetState.pressed,
        }, effect: AppAvatarEffect.none).scale,
        1.0,
      );
      expect(
        resolve(<WidgetState>{
    ...
    test('effect lift → 1.06 no hover / 1.02 no press', () {
      expect(
        resolve(<WidgetState>{
          WidgetState.hovered,
        }, effect: AppAvatarEffect.lift).scale,
        1.06,
      );
      expect(
        resolve(<WidgetState>{
    ...
    test('dragged → 1.08 vence qualquer effect', () {
      for (final effect in AppAvatarEffect.values) {
        expect(
  ...
  group('resolveAvatarInteraction · overlay (escada de prioridade única)', () {
    test('pressed → 0.12', () {
      expect(
  ...
  group('resolveAvatarInteraction · liftShadow', () {
  ...
  group('AppAvatarSize', () {
    test('baseDiameter == contentSize + 2·padding (32/48/64/96)', () {
      expect(AppAvatarSize.s.baseDiameter, 32);
      expect(AppAvatarSize.m.baseDiameter, 48);
      expect(AppAvatarSize.l.baseDiameter, 64);
      expect(AppAvatarSize.xl.baseDiameter, 96);
    });

    test('contentSize == iconSize.value', () {
      for (final size in AppAvatarSize.values) {
        expect(size.contentSize, size.iconSize.value);
      }
    });
  });
}
