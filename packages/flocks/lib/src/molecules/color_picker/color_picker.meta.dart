...
const AppComponentMeta appColorPickerPanelMeta = AppComponentMeta(
  ...
  props: <PropMeta>[
    ...
    PropMeta(
      ...
      defaultValue: 'true',
      description: LocalizedText(
        en: 'Shows the free-editing spectrum (saturation area, hue bar and the hex preview). False = presets-only, the fixed-palette picker; at least one of the two sections must stay on (asserted).',
        pt: 'Mostra o espectro de edição livre (área de saturação, barra de matiz e o preview com hex). False = só-presets, o seletor de paleta fixa; pelo menos uma das duas seções fica ligada (assert).',
      ),
    ),
  ],
