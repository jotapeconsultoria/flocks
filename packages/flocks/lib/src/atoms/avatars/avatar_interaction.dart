...
/// Micro-animação de hover/press aplicada a um [AppAvatar] clicável.
enum AppAvatarEffect {
...
/// Resolução visual de um conjunto de estados de interação de um avatar:
/// a escala, o realce translúcido (overlay) e a sombra de "lift" (arraste).
///
/// - `scale`: fator de escala aplicado ao avatar.
...
/// um avatar. Sem dependência de `BuildContext` — testável diretamente (espelha
...
AvatarInteraction resolveAvatarInteraction({
  required Set<WidgetState> states,
  required AppAvatarEffect effect,
  ...
  final double scale = !motionEnabled
      ...
      ? 1.08
      : switch (effect) {
          AppAvatarEffect.none => 1.0,
          AppAvatarEffect.scale => pressed ? 0.94 : 1.0,
          AppAvatarEffect.lift =>
            pressed
                ? 1.02
                : hovered
                ? 1.06
