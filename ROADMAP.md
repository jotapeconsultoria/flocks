...
Atualizado em 2026-08-13 — o que cada fase mediu está na seção dela.
...
| A3 — o corte `0.1.2` | ✅ **publicado em 2026-08-12**: `flocks`, `flocks_phosphor`, `flocks_material` e `flocks_mcp` em 0.1.2, e `flocks_cupertino` e `flocks_lucide` estreando em 0.1.0 — os **seis** sob o publisher verificado `jotapeconsultoria.com.br` e os **seis** em **160/160** na pana, medidos na API do pub.dev. Tag `v0.1.2` na main, e o Release dela leva os três anexos que a distribuição MCP usa: `flocks-mcp.mcpb`, `flocks-mcp.mcpb.sha256` e `server.json`. No MCP Registry a `0.1.2` responde `isLatest: true`, e o `fileSha256` que ela declara bate com o sidecar `.sha256` **e** com o `shasum -a 256` do `.mcpb` baixado do Release — três lugares, um número. A entrada do registry é de 2026-08-13, um dia depois do corte no pub.dev, e a defasagem tem causa: a tag **gera e anexa** o `server.json`, mas nenhum job o submete — `mcp-publisher publish` é passo à mão, documentado no README do pacote. São dois canais, e só um deles a tag fecha |
...
| D — demo | ✅ **no ar em 2026-08-10** em `flocks.live/demo/`: pacote `flocks_demo` (membro do workspace, `publish_to: none` — é vitrine, não biblioteca), dashboard e CRUD só de componentes Flocks, estado na URL, logo client-side, e o `toDartSnippet` que escreve a marca em Dart colável. Deployada pelo CI deste repo, sob prefixo próprio da zone do site, com casca semântica indexável. **A pendência fechou com a 0.1.2**: o snippet emite `flippedSwatch`, `swatchFromSeed` e `neutralSwatchFromSeed`, que a `0.1.2` publicada exporta por `lib/tokens.dart` — o artefato de `test/support/` passa `dart analyze` num pacote descartável fora do workspace que declara `flocks: 0.1.2` hospedado (`source: hosted` no lock). Quem responde por isso em cada push é o job `snippet (hosted)`, e ele **pulou** no run do próprio corte: rodou às 20:17Z, cinco minutos antes do publish das 20:22Z, e disse no log o que deixou de conferir em vez de passar verde calado. No primeiro run depois do publish ele voltou a valer e conferiu: `flocks 0.1.2 está no pub.dev` → `No issues found!` |
