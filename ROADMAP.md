...
O pacote está maduro: 132 componentes migrados, 25 suítes de arquitetura, a
...
| B — Widgetbook hospedado | ✅ no ar em `widgetbook.flocks.live` (2026-08-06): release de 43 MB com `main.dart.js` de 4,2 MB contra 58 MB do debug; deploy no CI com `needs` nos dois gates e purge; contagem de use cases virou artefato gerado com gate (`doc/widgetbook_use_cases.json`, 299) |
...
| Validador | `cd packages/flocks && dart run tool/validate_components.dart` | 132 migrados + 7 internos |
...
| Use cases do Widgetbook | `grep -rc '@widgetbook.UseCase' widgetbook/use_cases/` | 299 |
...
components"; são 132. O `catalog_freshness_test` fiscaliza o README, não o
...
- **Empregos distintos, uma fonte só.** A galeria do site é documentação —
  ...
  demonstração viva de que três eixos restilizam 132 componentes. Nada é
...
O dado está pronto: `packages/flocks/doc/mcp/catalog.json`, 132 componentes,
